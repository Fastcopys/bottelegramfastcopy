...
async def check_user_limit(user_id: int) -> bool:
    ...
    allowed = 100 + user_searches[user_id]['granted']
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    remaining = 100 + user_searches[user_id]['granted'] - user_searches[user_id]['count']
...
async def show_user_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    remaining = 100 + data['granted'] - data['count']
    
    status_text = (
        f"📊 *Tu Estado*\n\n"
        f"🆔 ID de Usuario: `{user_id}`\n"
        f"🔍 Búsquedas realizadas: {data['count']}\n"
        f"🎯 Límite total: {100 + data['granted']}\n"
...
async def show_admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id, data in user_searches.items():
        stats_text += (
            ...
            f"- Disponibles: {15 + data['granted'] - data['count']}\n\n"
        )
